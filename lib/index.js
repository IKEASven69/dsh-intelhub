...
var KbStore = class {
  ...
  updateStageFields(fileId, chunks, stage) {
...
_computedKey = Service.init, _dec = Remote("status"), _dec1 = Remote("list"), _dec2 = Remote("import"), _dec3 = Remote("remove"), _dec4 = Remote("search"), _dec5 = Remote("url-import"), _dec6 = Remote("note"), _dec7 = Remote("upload"), _dec8 = Remote("demo"), _dec9 = Remote("workspace-add"), _dec10 = Remote("workspace-remove"), _dec11 = Remote("workspace-list"), _dec12 = Remote("today"), _dec13 = Remote("schedule-list"), _dec14 = Remote("schedule-set"), _dec15 = Remote("schedule-remove"), _dec16 = Remote("schedule-toggle");
...
${rows.join("\n")}`
    ...
    this.ctx.tools.register(defineTool({
      ...
      execute: async (a) => {
        ...
        const type = [
          ...
          "preference",
          "lesson"
        ].includes(String(a.type)) ? String(a.type) : "fact";
    ...
    this.ctx.tools.register(defineTool({
      ...
      parameters: {
        ...
        path: {
          type: "string",
          description: "promote/skip \u7684\u76EE\u6807\u8DEF\u5F84(\u6216\u552F\u4E00\u540E\u7F00)"
        }
      },
      output: {
        schema: {
      ...
      execute: async (a) => {
        ...
        const raw = [
          ...this.registry.values()
        ].filter((f) => f.meta?.stage === "raw").sort((x, y) => (y.meta?.likes ?? 0) - (x.meta?.likes ?? 0));
        if (action === "list") {
          if (raw.length === 0) return {
            text: "\u5F85\u5206\u8BCA\u961F\u5217\u4E3A\u7A7A\u3002"
          };
          return {
            text: `\u5F85\u5206\u8BCA ${raw.length} \u7BC7(\u6309\u8D5E\u964D\u5E8F):
  ...
  enqueueSource(key, display, bytes, produce) {
    ...
    this.queueTail = this.queueTail.then(async () => {
      ...
      try {
        ...
        this.ingestEvents.unshift({
          at: Date.now(),
          src,
          display,
          chunks: this.registry.get(key)?.chunks ?? 0,
          status: "done",
          error: "",
...
`),
  ...
  /** 分诊:热更新某来源全部块的 stage 标量(zvec updateSync,不重嵌入)。 */
  async setStage(path, stage) {
    await this.loadRegistry();
    const key = normKey3(path);
    let entry = this.registry.get(key);
    if (entry === void 0) {
      const lower = path.toLowerCase();
      const matches = [
        ...this.registry.values()
      ].filter((f) => f.path.toLowerCase().endsWith(lower) || f.path.toLowerCase() === lower);
      if (matches.length === 1) entry = matches[0];
      else if (matches.length > 1) return {
        ok: false,
        error: `\u8DEF\u5F84\u4E0D\u552F\u4E00(${matches.length} \u4E2A\u5339\u914D)`
      };
    }
    if (entry === void 0) return {
      ok: false,
  ...
  async rpcToday() {
    ...
    const top = [
      ...todayNew
    ].filter((f) => (f.meta?.likes ?? 0) > 0).sort((x, y) => (y.meta?.likes ?? 0) - (x.meta?.likes ?? 0)).slice(0, 8).map((f) => ({
      author: f.meta?.author ?? f.path.split(/[/\\]/).pop() ?? f.path,
      likes: f.meta?.likes ?? 0,
      file: f.path
    }));
    ...
    const lines = [
      ...
      ...top.length > 0 ? [
        "\u9AD8\u4EF7\u503C TOP:"
      ].concat(top.map((t) => `\u25C6 ${t.author} \xB7 \u8D5E ${t.likes} \xB7 ${t.file.split(/[\\/]/).pop()}`)) : []
    ];
    return {
      ok: true,
      date,
      newFiles: todayNew.length,
      rawPending,
      top,
      text: lines.join("\n")
    };
