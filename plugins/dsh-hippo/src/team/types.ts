...
export type LedgerEvent =
  ...
  | { kind: 'promo/candidate'; at: number; candidate: PromotionCandidate }
  | { kind: 'promo/resolved'; at: number; id: string; action: 'promoted' | 'ignored' | 'kept-private' }
  | { kind: 'whitelist/add'; at: number; text: string }
  | { kind: 'mem/decayed'; at: number; id: string; from: number; to: number }
...
export interface ScanReport {
  ...
  /** T2 起：自动晋升改为入队列，由面板审批。 */
  promotionQueued: number
}

/** 退役分流三档（v2 定稿：按忘掉的错误成本分级）。 */
export type RetireTier = 'auto' | 'approval' | 'evidence'
