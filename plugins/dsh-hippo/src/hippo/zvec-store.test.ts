/**
 * Tests for ZvecStore — the zvec-backed storage layer.
 *
 * Mirrors store.test.ts (which tests SqliteStore) but against zvec. Uses
 * orthogonal unit vectors for exact similarity assertions.
 */
...
import { EmbedModelMismatch, ZvecStore } from './zvec-store.js';
import type { MemoryRecord } from './memory.js';

const DIM = 8;

function tmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'hippo-zvec-'));
}
...
test('upsert/search roundtrip: identical vector has similarity ~1', async () => {
  const dir = tmpDir();
  const store = new ZvecStore(dir, DIM);
  try {
    ...
    assert.ok(Math.abs(hits[0][1] - 1.0) < 1e-3, `sim=${hits[0][1]}`);
  } finally {
    store.close();
...
test('upsert replaces text and vector on conflict', async () => {
  ...
  try {
    ...
    assert.ok(Math.abs(hits[0][1] - 1.0) < 1e-3);
  } finally {
    store.close();
    fs.rmSync(dir, { recursive: true, force: true });
  }
...
test('hybridSearch fuses FTS + vector via RRF', async () => {
  const dir = tmpDir();
  const store = new ZvecStore(dir, DIM);
  try {
...
test('dimension mismatch on reopen raises EmbedModelMismatch', async () => {
  ...
  store.close();
  try {
    assert.throws(() => new ZvecStore(dir, DIM * 2), EmbedModelMismatch);
    // reopening with the same dim is fine
    const again = new ZvecStore(dir, DIM);
