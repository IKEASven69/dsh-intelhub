...
esbuild.buildSync({
  ...
  platform: 'node',
  target: 'es2022',
  // 原生模块(zvec/onnxruntime)与重型解析器(pdfjs/mammoth)必须留在依赖里按需 require,不进 bundle。
  external: ['@deepseek-ai/*', 'react', 'react/*', '@zvec/*', '@huggingface/*', 'onnxruntime-*', 'sharp', 'pdfjs-dist', 'pdfjs-dist/*', 'mammoth', 'mammoth/*'],
